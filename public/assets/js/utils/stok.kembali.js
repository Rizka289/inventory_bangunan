async (e, dt, node, config) => {
    ...
    if (data.length == 0) {
        alert('Pilih Data yang ingin dikembalikan');
        $(node).prop('disabled', false);
        return;
    }
    var res = confirm('Yakin Ingin Mengembalikan Data .?');
    if (!res) {
        $(node).prop('disabled', false);
        return;
    }
    ...
    var inputs = [];
    var barang = data.map(d => [d[index_id], d[2] + ' ' + d[3] + '(' + d[4] + ') yang akan dikembalikan', d[7], d[8]]);
    if(custom){
        ...
        barang = data.map(d => [d[id], d[material] + ' ' + d[merk] + '(' + d[uom] + ') yang akan dikembalikan', d[total]]);
    }
    barang.forEach(b => {
        inputs.push({
        ...
        inputs.push({
            type: 'hidden', value: b[3], name: 'ids_bangunan['+ b[0] +']'
        });
        inputs.push({
            type: 'hidden', value: b[0], name: 'ids_stok[]'
        });
